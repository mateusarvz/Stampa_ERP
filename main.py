...
class StampaApp:
    ...
    def create_clients_tab(self):
        ...
        self.add_client_button = ttk.Button(form_inner, text="Adicionar cliente", command=self.add_client, style="Primary.TButton")
        self.add_client_button.grid(row=2, column=0, columnspan=4, padx=6, pady=(14, 6), sticky="ew")
    ...
    def create_orders_tab(self):
        ...
        self.add_order_button = ttk.Button(frame_form, text="Adicionar pedido", command=self.add_order, style="Primary.TButton")
        self.add_order_button.grid(row=3, column=1, padx=6, pady=8, sticky="w")
    ...
    def load_data(self):
        ...
        self.update_client_add_button_state()
        self.update_order_add_button_state()
    ...
    def create_ai_agent_tab(self):
        ...
        self.ai_agent_output = tk.Text(frame, height=20, bg="#fafafa", font=("Segoe UI", 10), wrap="word", padx=12, pady=12)
...
'''
    ...
    def _render_markdown_in_text(self, text_widget, markdown_content):
        """Render Markdown content in Text widget with formatting."""
        text_widget.delete("1.0", "end")
        
        # Configure tags
        text_widget.tag_config("heading1", font=("Segoe UI", 14, "bold"), foreground="#1a73e8")
        text_widget.tag_config("heading2", font=("Segoe UI", 12, "bold"), foreground="#1a73e8")
        text_widget.tag_config("heading3", font=("Segoe UI", 11, "bold"), foreground="#1a73e8")
        text_widget.tag_config("bold", font=("Segoe UI", 10, "bold"))
        text_widget.tag_config("list_item", lmargin1=20, lmargin2=30)
    ...
    def execute_gemini_prompt(self):
        ...
        system_instructions = """Você é um analista de dados especialista da empresa. Seu trabalho é entender o que o usuário (Darlan) está pedindo e gerar as queries SQL CORRETAS e COMPLETAS necessárias.

IMPORTANTE - DADOS DISPONÍVEIS:
- Você TEM ACESSO COMPLETO a duas tabelas: clientes e pedidos
- Todos os dados de clientes estão na tabela 'clientes' (nomes, emails, telefones, datas)
- Todos os pedidos estão na tabela 'pedidos' com datas e valores
- Use LEFT JOIN ou INNER JOIN para conectar tabelas quando necessário
- Datas estão em formato TEXT - use conversão se necessário

REGRAS SQL:
- Gere queries que funcionem em SQLite
- Sempre use nomes de colunas corretos (id_clientes, ClienteEmpresa, data_criacao, etc)
- Para análise temporal, converta datas: DATE(data), STRFTIME, SUBSTR
- Para agregações: SUM, COUNT, AVG, GROUP BY

RESPOSTA FINAL:
- Você NÃO deve mencionar código SQL, banco de dados ou termos técnicos na resposta final
- Se não entender a pergunta, peça esclarecimento natural e profissional
- Sempre responda como um colega da empresa

Gere um JSON com:
- gemini_json_gerado: array de queries SQL (completas e corretas)
- gemini_porque_gerou_json: razão simples das queries (sem termos técnicos)

Responda APENAS JSON."""
        
        user_msg_1 = f"""SCHEMA BANCO DADOS (SQLite):

Tabela 'clientes':
- id_clientes: ID único cliente
- ClienteEmpresa: nome empresa cliente
- email: email contato
- telefone: telefone contato
- data_criacao: data quando cliente foi criado (formato: YYYY-MM-DD ou DD/MM/YYYY)

Tabela 'pedidos':
- id_pedidos: ID único pedido
- id_cliente: referência ID cliente
- cliente_empresa: nome empresa (duplicado de clientes)
- quantidade: número itens (INTEGER)
- valor: valor total pedido (REAL, em reais)
- data: data pedido (formato: YYYY-MM-DD ou DD/MM/YYYY)
- descricao: descrição ou tipo pedido
- status: "Aguardando aprovação", "Em andamento", "Finalizado", "Cancelado"

IMPORTANTE: Você TEM ACESSO TOTAL aos dados. Use JOIN para conectar tabelas. Extraia datas corretamente.
...
Pergunta do Darlan: {prompt}"""
        ...
        try:
            ...
            self.ai_agent_output.insert("1.0", f"Erro ao processar: {e}\n")
            return
        ...
        if not json_text:
            self.ai_agent_output.delete("1.0", "end")
            self.ai_agent_output.insert("1.0", f"{resp1_text}")
            return
        ...
        if not isinstance(queries, list) or not queries:
            self.ai_agent_output.delete("1.0", "end")
            self.ai_agent_output.insert("1.0", "Não consegui gerar a análise. Por favor, reformule sua pergunta.")
            return
        ...
        for idx, q in enumerate(queries):
            try:
                rows = run_sql_query(q)
                query_results[f"query_{idx}"] = {"sql": q, "rows": rows, "rows_count": len(rows)}
            except Exception as e:
                has_errors = True
                query_results[f"query_{idx}"] = {"sql": q, "error": str(e), "error_type": type(e).__name__}

        # Step 3: Get final answer
        system_final = """Você é um analista de dados da empresa. Recebeu os resultados das análises.

REGRAS - OBRIGATÓRIO:
- NÃO inicie com saudações (sem "Olá", "Oi", "Recebi", "Analisei", etc)
- NÃO explique o processo de análise ou queries
- NÃO mencione SQL, banco de dados ou termos técnicos
- Vá DIRETO AO PONTO: exiba apenas os resultados e insights
- Se houver dados, apresente com **destaque** e números
- Se erro, avise brevemente qual é o problema
- Use Markdown: #título, **negrito**, -listas
- Seja conciso e objetivo"""
        ...
        try:
            ...
            self.ai_agent_output.delete("1.0", "end")
            self.ai_agent_output.insert("1.0", f"Erro ao gerar resposta: {e}\n")
            return

        # Display results: final answer only
        try:
            self._render_markdown_in_text(self.ai_agent_output, resp2_text)
        except Exception as e:
            self.ai_agent_output.delete("1.0", "end")
            self.ai_agent_output.insert("1.0", f"Erro ao renderizar: {str(e)}\n\nConteúdo original:\n{resp2_text}")
    ...
    def refresh_dashboard(self):
        ...
        # Count only months with actual data
        months_with_data = len(revenue_map)
        average_per_month = total_revenue / months_with_data if months_with_data > 0 else 0.0
    ...
    def update_client_add_button_state(self):
        """Enable/disable add client button based on selection state"""
        if self.selected_client_id:
            self.add_client_button.state(["disabled"])
        else:
            self.add_client_button.state(["!disabled"])

    def update_order_add_button_state(self):
        """Enable/disable add order button based on selection state"""
        if self.selected_order_id:
            self.add_order_button.state(["disabled"])
        else:
            self.add_order_button.state(["!disabled"])
